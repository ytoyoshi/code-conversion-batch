...
| FILE_C | 380バイト固定 | 1バイト+2バイト混合、4種レコード構成 | 部分変換(フィールド単位) |
| FILE_D | 380バイト固定 | 1バイト+2バイト混合、4種レコード構成 | 部分変換(フィールド単位、データ種別対応) |
...
- レコード構成:
  - ヘッダーレコード(1バイト目='1'): 1バイト文字として全体変換
  - データレコード(1バイト目='2'): フィールド単位で変換
  - トレーラーレコード(1バイト目='8'): 1バイト文字として全体変換
  - エンドレコード(1バイト目='9'): 1バイト文字として全体変換
  - 漢字フィールド位置(バイトベース):
    - 50-99バイト目 (25文字)
    - 150-199バイト目 (25文字)
    - 300-349バイト目 (25文字)
...
- レコード構成:
  - ヘッダーレコード(1バイト目='1'): 1バイト文字として全体変換
  - データレコード(1バイト目='2'): 2バイト目のデータ種別により処理が変わる
  - トレーラーレコード(1バイト目='8'): 1バイト文字として全体変換
  - エンドレコード(1バイト目='9'): 1バイト文字として全体変換
  - データ種別='1'の場合(バイトベース):
    - 100-149バイト目 (25文字)
...
UTF-8は可変長エンコーディングのため、固定長ファイル(FILE_C, FILE_D)の処理時は**文字数ベース**でレコード読み込みとフィールド抽出を行います。

#### レコード読み込み

FILE_C/D で UTF-8 を入力とする場合、レコードは**文字数**で区切られます。

- **UTF-8 入力**: 380**文字**で1レコード（バイト数は可変）
- **JIS/EBCDIC 入力**: 380**バイト**で1レコード（固定長）

#### フィールド抽出

- UTF-8入力時は**文字位置**でフィールドを抽出
- JIS/EBCDIC入力時は**バイト位置**でフィールドを抽出

#### 前提条件

- UTF-8ファイルには**漢字が含まれない**(英数字・半角カナのみ)
- UTF-8入力ファイルは380**文字**で構成
- JIS/EBCDIC出力ファイルは380**バイト**固定長
